  ...
  <div class="columns is-marginless pdfmain" style="height: 100vh; width: 100vw" @click="pageclick">
    <template v-if="requestId">
      ...
      <template v-else>
        <template v-if="pdfLoaded">
            ...
            <template v-if="pdfMode !== 'template'">
              ...
              <div v-if="!joinedAsWitness" id="notorizationTypeCard" class="q-px-md">
                ...
                <div v-if="pdfMode !== 'template' && sessionid !== 'simulator'" class="columns is-multiline" style="margin-top: 12px;">
                  <div v-if="witnessButtonLabel !== 'false' && witnessButtonLabel !== false" class="col-md-2 col-lg-2 col-xs-12" style="font-weight: bold">
              ...
              <div v-if="!joinedAsWitness && !($user.role === 'customer' && sessionChargeOnBusinessUser)" id="notorizationPriceCard" class="q-pa-md">
                <h2 class="q-my-md" style="font-size:1rem">
                  Cost of Notarization
                  <q-icon v-if="$user.role === 'notary' && $user.memberType !== 'free'" name="add_circle_outline" style="cursor: pointer; color: blue; font-size: 20px; float: right" @click="editCostOfNotarizationClicked">
                    <q-tooltip>
          ...
          <!-- Pdf Pages -->
          <div class="column is-8 pdfprvbox q-pt-md" style="padding-left:0;padding-right:0">
            <div
              id="upper-text"
              class="pdf-upper-text bg-white row justify-between flex-wrap"
            >
              <div style="padding: 0 0.25rem" class="col-md-3 col-xs-12 column inline tersec">
                <span v-if="pdfMode !== 'template'" class="col" style="font-size:1.1rem;">Session {{ sessionCode }}&nbsp;<img src="~assets/recording-circle.gif" style="width:15px;" /></span>

                <span
                  v-if="$user.role === 'notary' && pdfMode !== 'template' && !joinedAsWitness && !minimizeWaitingRoomData && !waitingRoomModalShow"
                  class="col"
              ...
              <div v-if="pdfMode !== 'template' && sessionid !== 'simulator'" class="col-md-5 col-xs-12 inline" style="padding:2px;padding-left:15px;">
              ...
              <div v-if="!joinedAsWitness && !minimizeWaitingRoomData && !waitingRoomModalShow" style="padding: 0 0.25rem 0 0.75rem" class="col-md-4  col-xs-12">
                <button
                  v-if="$user.role === 'notary' && pdfMode !== 'template'"
                ...
                <div v-if="$user.role === 'customer' && customerTotalFields" style="padding: 15px; border: 1px solid black; border-radius: 5px;display: flex;justify-content: space-between;">
                  ...
                  <div style="display: flex;flex-direction: row-reverse;">
                    <q-icon name="arrow_forward" style="cursor: pointer; color: blue; font-size: 20px; float: right; padding-left: 10px; padding-top: 2px" @click="nextCustomerInput($event)">
                      <q-tooltip>
                        Go to next input
                      </q-tooltip>
                    </q-icon>
            ...
            <div v-if="minimizeWaitingRoomData && $user.role === 'notary'" id="minimizedLoadingScreen" style="margin: 6px 6px; padding: 6px; background: white">
              <div style="display: inline-block; font-weight: bold">Waiting for Signer to Join</div>
    ...
    <q-dialog v-model="showCheckUserDetailsModal">
      <q-card style="">
        ...
        <q-card-section class="q-pt-none">
          ...
          <div v-if="sessionid === 'simulator' && userDetailsModalFrontImageUrl" style="text-align: center">
    ...
    <q-dialog v-if="!minimizeWaitingRoomData" v-model="waitingRoomModalShow">
      <q-card style="min-width: 60%">
        <q-card-section>
          <h5 style="display: inline-block">Waiting Room</h5>
          <q-btn v-if="$user.role === 'notary'" flat round color="primary" icon="keyboard_arrow_down" style="display: inline-block; float: right; font-weight: bold; top: -15px; right: -15px; font-size: 18px" @click="minimizeWaitingRoom">
            <q-tooltip>Minimize Waiting Room</q-tooltip>
          </q-btn>
        </q-card-section>

        <q-card-section class="q-pt-none">
          <q-card v-if="showNotarizeNowButtonModal" class="my-card q-pa-lg">
            <q-card-section>
              <div class="text-h6">Waiting...</div>
              <div class="text-subtitle2">We've been waiting more than 3 minutes for the assigned online notary to join, but it seems the notary hasn't joined yet.</div>
              <div class="text-subtitle2">You can wait more or call for a different online notary to pickup your session now.</div>
              <div v-if="showNotarizeNowButtonSuccess" style="margin-top: 20px; font-weight: bold">We have sent an open call to all online notaries. A notary will join this session ASAP.</div>
            </q-card-section>
     ...
     allDroppedElementsDocIdWise: {
       handler(value) {
         ...
         this.pendingInputs = {
          ...
          if (this.sessionFullData && this.sessionFullData.newSessionModelData && this.sessionFullData.newSessionModelData.sessionChargeOnBusinessUser) {
            notaryFee = "0.00";
            extraSeal = "0.00";
            // extraSeal = "4.00";
          ...
          if (this.$user.testingacc) {
            if (this.pendingInputs.signatures) {
              ...
              this.showCompleteButtonInfoText += `${String(this.pendingInputs.signatures)} ${tempText} left from Signer`;
            }
            if (this.pendingInputs.text) {
              ...
              this.showCompleteButtonInfoText += `${String(this.pendingInputs.text)} ${tempText2} left from Signer`;
            }
          }
       },
       deep: true
  ...
  async mounted () {
    ...
    try {
      if (this.$user.first_name && this.$user.last_name) {
        userInitials = this.$user.first_name.substring(0, 1).toUpperCase() + this.$user.last_name.substring(0, 1).toUpperCase();
      } else {
          ...
          if (outputStatus === "success") {
            ...
            if (lastDocument) {
              ...
              setTimeout(() => {
                this.endVideoCall();
                this.completePDFLoadingModal = false;
                if (localStorage.getItem("userRole") === "witness") {
      ...
      this.$q.dialog({
        ...
        for (let documentIndex = 0; documentIndex < this.allDocumentList.length; documentIndex += 1) {
          ...
          pdfDoc.save().then(async (pdfBytes) => {
            ...
            // complete sessions
            console.log("fpr - complete session referral", this.$user.email, this.$user.id);
