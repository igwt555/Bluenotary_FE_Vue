  ...
  <q-page class="q-pa-md flex-center session-container">
    <div class="row">
      ...
      <div class="col-12 col-md-12 q-pa-lg workflow-container">
        ...
        <div v-if="notaryActiveCount >=1" class="col-12 col-md-8 flex q-mt-lg">
          <div class="flex column">
            <h3 class="no-margin">Notarize Timing</h3>
        ...
        <div v-if="showSelectNotaryPicker || ($user.memberType === 'free' && !(sessionData && sessionData.session && sessionData.session.notaryUserId))" class="col-12 col-md-8 flex">
          <div class="flex column" style="padding-top: 0px">
            <h6 class="no-margin"><strong>Notary Selection for Session</strong></h6>
            <div class="row doc-list-preview">
              <div class="">
        ...
        <div v-if="!skipCustomerKBACheck" class="col-12 col-md-8 flex q-mt-lg">
          <div class="flex column">
            <h3 class="no-margin">Type of Identity Authentication</h3>
            <div class="row doc-list-preview">
              <div class="q-pt-sm">
                ...
                <q-radio v-if="!hidePassportBiometricsWorkflow" v-model="typeOfKBA" val="foreigners_without_residential" label="US Citizens/Foreigners + Biometrics" />
              </div>
            </div>
            <div class="q-mx-sm">
              <q-icon name="info" color="blue" class="q-mr-xs" />
              <small><a href="https://bluenotary.freshdesk.com/support/solutions/articles/72000562604-types-of-identity-authentication" target="_blank"><u>More details on identification types</u></a></small>
            </div>
          </div>
        </div>
                ...
                <div style="width: 100%">
                  ...
                  >
                    <div class="d-flex justify-content-between w-100">
                      <div class=" space-8 space-5 text-wrapper"><span>{{ item.name }}</span></div>
                      <div class="space-5 space-2 text-right">
    ...
    <q-dialog v-model="openEncryptionFilesModal">
      <q-card>
        ...
        <q-card-actions align="right">
          <q-btn outline label="OK" v-close-popup color="primary" />
        </q-card-actions>
      </q-card>
    </q-dialog>
...
export default {
  name: "PrepareDoc",
  components: { VuePdfEmbed, SessionHeader, UpgradeAccountPopupComponent },
  data() {
    return {
      uploading: false,
      sessionid: "",
  ...
  watch: {
    ...
    assignNotary: {
      handler(value) {
        if (value === "choose_notary" && (this.$user.memberType === "free" || this.$user.memberType === "pro")) {
  ...
  async mounted () {
    ...
    if (this.sessionData && this.sessionData.notaryUserDoc) {
      if (this.sessionData.notaryUserDoc.state !== "Florida" && this.sessionData.notaryUserDoc.state !== "Virginia") {
